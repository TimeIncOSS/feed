...
function Feed(options) {
    ...
    this.customNamespace   = options.customNamespace;
    this.customNamespaceUri = options.customNamespaceUri;
...
function atom_1_0(options) {
    ...
    if (options.customNamespace) {
      var customNamespace = {}
      customNamespace[`xmlns:${options.customNamespace}`] = options.customNamespaceUri
      feed.push({ _attr: customNamespace })
    }
    ...
    for(var i = 0; i < entries.length; i++) {
        ...
        if(entries[i].link == undefined)
            throw new Error('entry needs a link');
        else
        ...
        var customElements = _.omit(entries[i], ['title', 'id', 'link', 'date', 'description', 'content', 'author', 'contributor', 'published', 'copyright'])
        pushCustomElements(customElements, entry, options.customNamespace)
...
function rss_2_0(options) {
    ...
    for(var i = 0; i < items.length; i++) {
        ...
        var customElements = _.omit(items[i], ['title', 'guid', 'link', 'date', 'description', 'content', 'author', 'image'])
        pushCustomElements(customElements, item, options.customNamespace)

        channel.push({ item: item });
    }

    if (options.customNamespace) {
        container[0].rss[0]._attr[`xmlns:${options.customNamespace}`] = options.customNamespaceUri
    }
...
function pushCustomElements(customElements, item, customNamespace) {
  _.map(customElements, function(value, key) {
    if (value !== undefined) {
      if (_.isArray(value)) {
        ...
        var properties = []
        _.map(value, function(val, prop) {
          console.log(`${prop} is ${val}`)
          pushTag(prop.replace('$', ''), val, properties)
          console.log(properties)
        })
        pushTag(key, properties, item)
      } else {
        pushTag(key, value, item)
      }
    }
  })

  function pushTag (key, value, list) {
    var tag = `${customNamespace}:${key}`
    var newObj = {}
    newObj[tag] = value
    list.push(newObj)
  }
